...
export default function parseExecs(execs: Exec[]): ExecData[] {
    return execs.map((doc): ExecData => {
        ...
        const imageUrl =
            doc.image && typeof doc.image === 'object' && 'url' in doc.image && typeof doc.image.url === 'string'
                ? doc.image.url
                : '/images/logo/esa_mascot.png';

        // Safe check for image alt text
        const imageAlt =
            doc.image && typeof doc.image === 'object' && 'alt' in doc.image && typeof doc.image.alt === 'string'
                ? doc.image.alt
                : 'Executive Image';
