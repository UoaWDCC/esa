...
const Carousel: React.FC<CarouselProps> = ({ Data, onCenterChange }) => {
  ...
  const [preShiftCount, setPreShiftCount] = useState(0);
  ...
  useEffect(() => {
    if (hasInitialized && preShiftCount < 3) {
      // Do 3 silent shifts to pre-render all images in all positions
      const timer = setTimeout(() => {
        ...
        setPreShiftCount(prev => prev + 1);
      }, 0);
      return () => clearTimeout(timer);
    }
  }, [hasInitialized, preShiftCount, Data.length]);
  ...
  if (!Data || Data.length === 0 || !hasInitialized) return null;
