...
export default function RetailOtherFolder({ isActive, onActivate }: RetailProps) {
    ...
    return (
        ...
        >
            {/* Angled tab */}
            <div
                className={`relative z-10 w-40 sm:w-56 md:w-[300px] h-10 sm:h-11 md:h-12 bg-[#55A077] flex items-center px-2 sm:px-4 justify-center ml-10 sm:ml-20 md:ml-40 lg:ml-224 transition-all duration-300 ease-in-out ${tabAnimationClasses}`}
                    ...
                    <div className="w-full h-full p-8 rounded-2xl no-scrollbar overflow-y-auto">
                        <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 lg:gap-10">
                            {sponsors && sponsors.length > 0 ? (
                                sponsors
                                    .filter((s) => s.type === 'Retail Other' || !s.type)
                                    .map((s) => (
                                        <SponsorCard
                                            key={s.id}
                                            name={s.name}
